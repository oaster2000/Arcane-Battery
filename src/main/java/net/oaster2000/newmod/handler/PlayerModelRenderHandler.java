...
public class PlayerModelRenderHandler {
	...
	public void renderModel(RenderPlayerEvent.Pre event) {
		if (event.getEntityPlayer().getHeldItemMainhand().getItem() instanceof SpellItem || event.getEntityPlayer().getHeldItemOffhand().getItem() instanceof SpellItem) {
	...
	public void renderModel(RenderPlayerEvent.Post event) {
		if (event.getEntityPlayer().getHeldItemMainhand().getItem() instanceof SpellItem || event.getEntityPlayer().getHeldItemOffhand().getItem() instanceof SpellItem) {
			...
			event.getRenderer().getMainModel().bipedLeftArm.rotationPointX = -MathHelper.cos((float) Math.toRadians(player.renderYawOffset)) * -5.0f;
