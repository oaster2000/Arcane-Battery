...
public class ResearchUpdateHandler {
	...
	public void pickupItem(PlayerTickEvent event) {
		...
		if (ResearchList.start.getState().equals(EnumResearchState.FOUND)) {
			ItemStack stackO = new ItemStack(ModBlocks.obscural);
			if (event.player.inventory.hasItemStack(stackO) && ResearchList.obscural.getState().equals(EnumResearchState.UNDISCOVERED)) {
				if(ResearchList.obscural.shouldShowToast()) {
					Minecraft.getMinecraft().getToastGui().add(new ResearchToast(ResearchList.obscural));
					ResearchList.obscural.setShowToast(false);
				}
				ResearchList.obscural.setState(EnumResearchState.FOUND);
				for (Research children : ResearchList.obscural.getChildren()) {
					children.setState(EnumResearchState.UNDISCOVERED);
				}
			}
		}

		if (ResearchList.obscural.getState().equals(EnumResearchState.FOUND)) {
